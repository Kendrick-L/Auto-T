...
- 新窗口或新模型接手前必须先读 `docs/PROJECT_HANDOFF.md`；Phase 2 接手可直接使用 `docs/PHASE_2_NEW_WINDOW_PROMPT.md`。
...
| 启用/暂停状态 | 插件应能停止自动/手动翻译，并让用户从浏览器图标看出当前状态。 | Popup 支持 Pause/Resume，暂停时 Chrome 图标切换并显示 `OFF` badge。 | M1 已覆盖，M3 可继续做浮球状态。 |
| 页面侧边快捷浮球 | 官网文档展示网页右侧快捷翻译图标，可直接触发页面翻译。 | 未实现。 | M3 交互层。 |
| 右键菜单 | 支持右键翻译网页/显示原文。 | 未实现。 | M3 交互层。 |
| 快捷键 | 默认 `Alt+A` 翻译/显示原文，`Alt+W` 整页翻译，支持设置页修改。 | 已支持 `Option+V` 可视区域翻译、`Option+T` 选区优先上下文翻译，Options 可查看 Chrome commands 绑定状态并打开快捷键设置。 | M3 交互层。 |
...
| 能力 | 沉浸式翻译表现 | Auto-T 当前状态 | Roadmap 对齐 |
| --- | --- | --- | --- |
| 鼠标悬停翻译 | 开启后，悬停段落并按快捷键显示段落译文。 | 已支持通过 `Option+T` 翻译最近鼠标指向的句子/段落；重复触发同一目标会关闭译文，不会单纯 hover 自动请求。 | M3。 |
| 划词翻译 | 选中单词或短语即可查看 AI 翻译和发音。 | 已支持通过 `Option+T` 翻译选中文本，并带所在段落上下文；交互译文可关闭，浮窗、发音和复制仍未做。 | M3。 |
...
- 增加全局 Pause/Resume 控制和 Chrome action 图标状态，避免不想翻译时继续触发请求。
...
- 用户可以清理全部缓存或清理当前站点缓存；后续补站点禁用缓存。
- 用户可以选择 DeepSeek 模型，切换模型不会误用旧模型缓存。
...
- 划词翻译浮窗。当前已完成快捷键选区翻译，结果插入页面内；浮窗样式、发音和复制后续补。
- 悬停段落翻译。当前已完成 `Option+T` 最近鼠标位置句子/段落翻译，支持同目标重复触发关闭；纯 hover 自动触发不做默认行为。
- 快捷键翻译当前选区。已完成 `Option+T`，选区优先于鼠标段落。
- 快捷键翻译可视区域。已完成 `Option+V`。
- 页面侧边快捷浮球。
...
- 选中文本后可通过快捷键翻译并插入结果；浮窗展示后续补。
- 浮窗不遮挡原文选择区域。
- 快捷键可在 Options 中查看绑定状态，并跳转到 Chrome 原生快捷键页面配置。

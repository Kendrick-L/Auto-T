...
| 能力 | 沉浸式翻译表现 | Auto-T 当前状态 | Roadmap 对齐 |
| --- | --- | --- | --- |
| 鼠标悬停翻译 | 开启后，悬停段落并按快捷键显示段落译文。 | 已支持通过 `Option+T` 翻译最近鼠标指向的句子/段落；不会单纯 hover 自动请求。 | M3。 |
| 划词翻译 | 选中单词或短语即可查看 AI 翻译和发音。 | 已支持通过 `Option+T` 翻译选中文本，并带所在段落上下文；浮窗、发音和复制仍未做。 | M3。 |
...
- 悬停段落翻译。当前已完成 `Option+T` 最近鼠标位置句子/段落翻译；纯 hover 自动触发不做默认行为。
